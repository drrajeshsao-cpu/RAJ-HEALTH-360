...
let routineStatementRows=[];
function statementCheckForDate(ds){ensureRoutineProtocol();return db.routineProtocol.dailyChecks.find(x=>x.date===ds)||null}
function statementRowForDate(ds){const mode=routineModeForDate(ds),p=routinePeriodForDate(ds),c=statementCheckForDate(ds);return {date:ds,mode,modeLabel:routineModeMeta(mode).label,reason:p?.reason||'',periodNote:p?.note||'',ahara:c?.ahara,nidra:c?.nidra,brahma:c?.brahma,overall:c?.overall,note:c?.note||''}}
function buildRoutineStatementRows(start,end){ensureRoutineProtocol();let s=start,e=end;const td=today();if(s<db.routineProtocol.trackingStart)s=db.routineProtocol.trackingStart;if(e>td)e=td;if(s>e)return[];const out=[];let d=s,guard=0;while(d<=e&&guard<4000){out.push(statementRowForDate(d));d=addDaysIso(d,1);guard++}return out}
function setRoutineStatementRange(days,btn){const to=today(),from=addDaysIso(to,-(days-1));$('rsFrom').value=from;$('rsTo').value=to;document.querySelectorAll('.period-buttons button').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');$('rsPeriodLabel').textContent=days===365?'Last 12 months':days===730?'Last 2 years':`Last ${days} days`;renderRoutineStatement()}
function avgStatement(rows,key){const a=rows.map(x=>Number(x[key])).filter(Number.isFinite);return a.length?Math.round(a.reduce((s,x)=>s+x,0)/a.length):null}
function statementModeCounts(rows){const c={};rows.forEach(r=>c[r.mode]=(c[r.mode]||0)+1);return c}
function renderRoutineStatement(){if(!$('rsFrom'))return;ensureRoutineProtocol();if(!$('rsFrom').value||!$('rsTo').value){$('rsTo').value=today();$('rsFrom').value=addDaysIso(today(),-29)}routineStatementRows=buildRoutineStatementRows($('rsFrom').value,$('rsTo').value);const c=statementModeCounts(routineStatementRows),std=c.standard||0,chg=routineStatementRows.length-std,pct=routineStatementRows.length?Math.round(std*100/routineStatementRows.length):0;$('rsTracked').textContent=routineStatementRows.length;$('rsStandard').textContent=std;$('rsChanged').textContent=chg;$('rsStandardPct').textContent=routineStatementRows.length?pct+'%':'--';const a=avgStatement(routineStatementRows,'ahara'),n=avgStatement(routineStatementRows,'nidra'),b=avgStatement(routineStatementRows,'brahma');$('rsAhara').textContent=a==null?'--':a+'%';$('rsNidra').textContent=n==null?'--':n+'%';$('rsBrahma').textContent=b==null?'--':b+'%';const modes=['standard','home','travel','exams','fasting','function','illness'];$('rsModeSummary').innerHTML=modes.map(m=>{const d=c[m]||0,p=routineStatementRows.length?Math.round(d*100/routineStatementRows.length):0,meta=routineModeMeta(m);return `<div class="mode-summary-card"><span>${meta.icon} ${meta.label}</span><b>${d} day${d===1?'':'s'}</b><small>${p}% of tracked period</small></div>`}).join('');const special=modes.slice(1).map(m=>({m,d:c[m]||0})).sort((x,y)=>y.d-x.d),top=special[0];$('rsDeviation').innerHTML=routineStatementRows.length?`Tracked <b>${routineStatementRows.length}</b> day(s). Standard Protocol: <b>${std}</b> (${pct}%). Changed routine: <b>${chg}</b> day(s).${top?.d?` Most frequent special mode: <b>${routineModeMeta(top.m).label}</b> — ${top.d} day(s).`: ' No special mode recorded in this period.'}`:'No routine history in this period.';const plan=[];if(routineStatementRows.length){if(pct>=96)plan.push('Excellent consistency: the 350/365 goal is broadly on track.');else if(pct>=90)plan.push('Good consistency; identify preventable variation days to move closer to 350/365.');else plan.push('Variation burden is relatively high; review recurring reasons and plan routine buffers.');if(c.travel)plan.push(`Travel: ${c.travel} day(s) — pre-plan sleep, hydration and meal availability.`);if(c.exams)plan.push(`Exams: ${c.exams} day(s) — protect recovery sleep and schedule a post-exam reset.`);if(c.fasting)plan.push(`Fasting/Ekadashi: ${c.fasting} day(s) — retain Parana timing and tolerance notes.`);if(c.function)plan.push(`Functions/festivals: ${c.function} day(s) — pre-plan the return-to-standard day.`);if(c.illness)plan.push(`Illness/recovery: ${c.illness} day(s) — distinguish medically necessary variation from lifestyle drift.`)}$('rsPlanning').innerHTML=plan.length?plan.map((x,i)=>`${i+1}. ${x}`).join('<br>'):'Add routine history to generate planning signals.';renderRoutineStatementTable();renderRoutineYearArchive()}
function renderRoutineStatementTable(){if(!$('rsTableBody'))return;const q=(v('rsSearch')||'').toLowerCase(),rows=routineStatementRows.filter(r=>`${r.modeLabel} ${r.reason} ${r.periodNote} ${r.note}`.toLowerCase().includes(q)).slice().reverse();$('rsTableBody').innerHTML=rows.map(r=>`<tr><td>${r.date}</td><td><b>${r.modeLabel}</b></td><td>${r.reason||r.periodNote||'—'}</td><td>${Number.isFinite(Number(r.ahara))?r.ahara+'%':'—'}</td><td>${Number.isFinite(Number(r.nidra))?r.nidra+'%':'—'}</td><td>${Number.isFinite(Number(r.brahma))?r.brahma+'%':'—'}</td><td>${Number.isFinite(Number(r.overall))?r.overall+'%':'—'}</td><td>${r.note||'—'}</td></tr>`).join('')||'<tr><td colspan="8">No records in selected period.</td></tr>'}
function renderRoutineYearArchive(){if(!$('rsYearArchive'))return;ensureRoutineProtocol();const startYear=Number(db.routineProtocol.trackingStart.slice(0,4)),endYear=Number(today().slice(0,4)),cards=[];for(let y=endYear;y>=startYear;y--){const s=`${y}-01-01`,e=`${y}-12-31`,rows=buildRoutineStatementRows(s,e);if(!rows.length)continue;const c=statementModeCounts(rows),std=c.standard||0,pct=Math.round(std*100/rows.length);cards.push(`<div class="year-card"><h4>${y}</h4><div class="minirow"><span>Tracked</span><b>${rows.length}</b></div><div class="minirow"><span>Standard</span><b>${std} (${pct}%)</b></div><div class="minirow"><span>Travel</span><b>${c.travel||0}</b></div><div class="minirow"><span>Home Visit</span><b>${c.home||0}</b></div><div class="minirow"><span>Exams</span><b>${c.exams||0}</b></div><div class="minirow"><span>Fasting</span><b>${c.fasting||0}</b></div><div class="minirow"><span>Functions</span><b>${c.function||0}</b></div><div class="minirow"><span>Illness</span><b>${c.illness||0}</b></div><div class="buttons" style="margin-top:8px"><button class="ghost" onclick="openRoutineYearStatement(${y})">Open year</button></div></div>`)}$('rsYearArchive').innerHTML=cards.join('')||'<p class="muted">Annual archive will appear as routine history accumulates.</p>'}
function openRoutineYearStatement(y){$('rsFrom').value=`${y}-01-01`;$('rsTo').value=`${y}-12-31`;$('rsPeriodLabel').textContent=`Year ${y}`;renderRoutineStatement();window.scrollTo({top:0,behavior:'smooth'})}
function routineStatementShareText(){const c=statementModeCounts(routineStatementRows),std=c.standard||0,pct=routineStatementRows.length?Math.round(std*100/routineStatementRows.length):0;return `RAJ HEALTH 360 — Lifestyle & Routine Statement\\nPeriod: ${$('rsFrom').value} to ${$('rsTo').value}\\nTracked: ${routineStatementRows.length} days\\nStandard Protocol: ${std} (${pct}%)\\nHome Visit: ${c.home||0}\\nTravelling: ${c.travel||0}\\nExams: ${c.exams||0}\\nFasting / Ekadashi: ${c.fasting||0}\\nFestival / Function: ${c.function||0}\\nIllness / Recovery: ${c.illness||0}\\nAhara avg: ${avgStatement(routineStatementRows,'ahara')??'NA'}%\\nNidra avg: ${avgStatement(routineStatementRows,'nidra')??'NA'}%\\nBrahmacharya avg: ${avgStatement(routineStatementRows,'brahma')??'NA'}%`}
async function shareRoutineStatement(){renderRoutineStatement();const text=routineStatementShareText();if(navigator.share){try{await navigator.share({title:'RAJ HEALTH 360 Lifestyle Statement',text});return}catch(e){}}window.open('https://wa.me/?text='+encodeURIComponent(text),'_blank')}
function printRoutineStatement(){renderRoutineStatement();window.print()}
function routineStatementCSV(){const head=['Date','Mode','Reason/Context','Ahara %','Nidra %','Brahmacharya %','Overall %','Note'],esc=x=>'"'+String(x??'').replaceAll('"','""')+'"';return [head,...routineStatementRows.map(r=>[r.date,r.modeLabel,r.reason||r.periodNote,r.ahara??'',r.nidra??'',r.brahma??'',r.overall??'',r.note])].map(row=>row.map(esc).join(',')).join('\\n')}
function downloadRoutineStatementCSV(){renderRoutineStatement();const blob=new Blob([routineStatementCSV()],{type:'text/csv;charset=utf-8'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`RAJ_HEALTH_360_Lifestyle_Statement_${$('rsFrom').value}_to_${$('rsTo').value}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000)}
function persist(){
 syncActiveMemberRecord();
 localStorage.setItem(KEY,JSON.stringify(db));
 renderAll();
...
["rsFrom","rsTo","routineStart","routineEnd","routineCheckDate","tdDate","labDate","imgDate","txDate","msDate","slDate","phDate","exStart","pvDue","vlDate","liDate"].forEach(id=>{if($(id))$(id).value=today()});
if($("rsFrom")){$("rsTo").value=today();$("rsFrom").value=addDaysIso(today(),-29)}
...
function renderAll(){
 safeRun("Lifestyle Statement",()=>{if($("routinestatement"))renderRoutineStatement()});
